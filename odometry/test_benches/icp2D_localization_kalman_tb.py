...
class icp2DLocalizationKalmanTB:
    ...
    def init_filter(self,
        ...
        # self.filter_R = np.diag([1,1,0.1]) ** 2 #original values
        self.filter_R = np.diag([0.02,0.02,0.005]) ** 2
    ...
    def get_dataset_start_time(self,idx=0)->float:
        ...
        if self.dataset.imu_full_enabled:
            data = self.dataset.get_imu_full_data(idx=0)
            start_time = max(data[0,0],start_time)
        
        if self.dataset.vehicle_vel_enabled:
            data = self.dataset.get_vehicle_vel_data(idx=0)
            start_time = max(data[0,0],start_time)
