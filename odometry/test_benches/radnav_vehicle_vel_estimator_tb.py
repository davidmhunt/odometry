...
from odometry.point_cloud_processing.vehicle_vel_estimator import VehicleVelEstimator
from odometry.plotting.movies import MovieGenerator

class RadnavVehicleVelEstimatorTB:

    def __init__(self,
                 localizer:icp2DLocalization,
                 gt_localizer:icp2DLocalization,
                 pc_stacker:temporalPcStacker,
                 vehicle_vel_estimator:VehicleVelEstimator,
        ...
        #initialize the vehicle velocity estimator
        self.vehicle_vel_estimator:VehicleVelEstimator = vehicle_vel_estimator
        ...
        self.history_vehicle_vel = None #stores [x,y]
        self.history_vehicle_vel_gt = None #only stores x
        self.history_vehicle_vel_reset()
    ...
    ####################################################################
    #Histories (vehicle velocity estimate)
    #################################################################### 


    def history_vehicle_vel_reset(self):

        n = self.dataset.num_frames
        
        self.history_vehicle_vel = np.zeros(shape=(n,2),dtype=np.double)
        self.history_vehicle_vel_gt = np.zeros(shape=n,dtype=np.double)

    def history_update_vehicle_vel(
            self,
            vel:np.ndarray,
            idx:int):
        """Save the vehicle history at the given index

        Args:
            vel (np.ndarray): the [x,y,z] velocity of the vehicle in 
                meters per seconds as estimated using radar
            idx (int): index of the sample from the dataset
        """
        
        self.history_vehicle_vel[idx] = vel

    def history_update_vehicle_vel_gt(
            self,
            vel:np.ndarray,
            idx:int):
        """Save the ground truth vehicle history at the given index

        Args:
            vel (np.ndarray): the [x,y,z] velocity of the vehicle in 
                meters per seconds as measured using ground truth
            idx (int): index of the sample from the dataset
        """
        
        self.history_vehicle_vel_gt[idx] = vel
    ...
    def run(
        ...
        for i in tqdm(range(max_frame)):
            ...
            #estimate the velocity of the vehicle
            vel_est = self.vehicle_vel_estimator.estimate_ego_vel(
                detections=radar_points,
                initial_ego_vel_est=np.array([self.filter.x[3],0.0]),
                only_2D=True
            )
            if vel_est.shape[0] > 0:
                self.history_update_vehicle_vel(
                    vel=vel_est,
                    idx=i
                )
            self.history_update_vehicle_vel_gt(
                vel=self.filter.x[3],
                idx=i
            )
