...
class temporalCombinedPCPFTb:
    ...
    def init_localization(self,
        ...
        self.localizer.initialize_from_gaussian_dist(
            mean=np.array([
                est_start_pose_m[0],
                est_start_pose_m[1],
                est_start_heading_rad
        ...
        #TODO: change to uniform if originally lost
    ...
    def history_save_particles_and_weights(self):
    ...
    def vehicle_vel_check_for_movement(self,vel_data:np.ndarray):
        """For a given velocity measurement, set the vehicle moving flag
            depending on if the vehicle is moving or not

        Args:
            vel_data (np.ndarray): Nx2 np array corresponding the [x,y] vel
        """
    ...
    def motion_model_predict_from_frame_samples(self, idx = 0):
        ...
        for i in range(imu_data.shape[0]):
            ...
            #predict the pc stacker forward
            ...
            #update the vehicle moving flag
    ...
    def run(
        ...
        for i in tqdm(range(max_frame)):
            ...
            #only update the pf if the vehicle is moving
            if self.vehicle_moving:
                ...
                if self.point_cloud_stacker.new_pc_available:
                    ...
                    #save what the particles would have looked like after the motion model
                    ...
                    self.latest_pose_m = self.localizer.current_pose_m
                    self.latest_heading_rad = self.localizer.current_heading_rad
    ...
    def plot_compilation(
        ...
        if combined_pc.shape[0] > 0 or dynamic_combined_pc.shape[0] > 0:
            ...
            self.plotter_localization.plot_dynamic_and_static_detections_on_map(
                ...
                pose_m=self.localizer.motion_model.x[0:2],
                ax=axs[1,2],
                show=False
            ) 
